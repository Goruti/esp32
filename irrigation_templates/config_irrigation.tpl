...
.selected_choice{ background-color: white;font-weight: bold}
    ...
    <h1>Irrigation System Configuration</h1>
      <form target="_blank" action="/irrigation_config_2" method="post">
...
<script>
  function totalPumpsFunction() {
    ...
    for (var i = 1; i <= total_pump; i++) {
      myHTML += '<h3>Pump #' + i + '</h3>'
      myHTML += 'Connected to Port:  <select input type="text" onchange="this.className=this.options[this.selectedIndex].className" name="connected_to_port_' + i + '" required>';
      ...
      myHTML += '<option value=G class="choice_E">E</option>'
      myHTML += '<option value=H class="choice_F">F</option>'
      myHTML += '</select><br><br>';
      myHTML += 'Threshold:  <input type="number" value="400" style="font-weight: bold" name="moisture_threshold_' + i + '" required><br><br>';
    }
    myHTML += `<input onClick="window.location='/';" type="submit" value="Save">`
